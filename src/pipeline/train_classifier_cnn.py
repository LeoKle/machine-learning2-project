...
def train_classifier_cnn(train_loader, test_loader, dummy_input, encoder_path):
    ...
    combined_model = CNNClassifier(dataset=dataset_type).to(DEVICE)

    optimizer = optim.Adam(combined_model.parameters(), lr=0.00001)
    # optimizer = optim.Adam(combined_model.parameters(), lr=0.0001, betas=(0.9, 0.999), weight_decay=1e-5)
    # optimizer = optim.Adam(combined_model.parameters(), lr=0.0001, weight_decay=1e-5)
