...
from pipeline.train_classifier_linear import train_classifier_linear, tune_hyperparameters
...
class TestLinearOnEncodedTraining(unittest.TestCase):
    ...
    def test_train_with_best_optuna_params(self):
        ...
        model, pipeline = train_classifier_linear(train_loader, test_loader, dummy_input, encoder_path, lr=lr)
        ...
        def plot_callback(metrics, epoch):
            Plotter.plot_loss_progression(metrics=metrics, epochs=list(range(1, epoch + 1)), output_file_name=plot_save_dir, dataset_type=dataset_type)
