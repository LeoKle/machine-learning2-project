...
class TestCNNTrainingWithOptuna(unittest.TestCase):
    def test_training_and_saving(self):
        ...
        train_loader, test_loader, dummy_input = prepare_dataset_for_cnn(dataset_type, batch_size)
        model, pipeline = train_classifier_cnn(train_loader, test_loader, dummy_input, lr=lr)
        ...
        Plotter.plot_metrics(pipeline.tracker.get_metrics(), plot_save_dir / "metrics.png")
        Plotter.plot_accuracy(
            pipeline.tracker.get_metrics()["accuracy"],
            plot_save_dir / "accuracy.png",
            dataset_type
        )
        Plotter.plot_predictions(model, test_loader, dataset_type, DEVICE, plot_save_dir / "predictions.png", show=False)

        if dataset_type == "MNIST":
            Plotter.plot_confusion_matrix_mnist(model, test_loader, DEVICE, plot_save_dir / "confusion_matrix.png")
        else:
            Plotter.plot_confusion_matrix_cifar10(model, test_loader, DEVICE, plot_save_dir / "confusion_matrix.png")
